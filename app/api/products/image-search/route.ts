...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      // Enhanced structured prompt for better beauty product identification
      const promptText = `You are a beauty product identification expert. Analyze this image of a beauty product and provide a detailed, structured response:
...
Focus carefully on reading all text visible on the packaging, including small print. If you cannot clearly see certain details, indicate this but make your best guess based on what is visible. If the image doesn't appear to show a beauty product, please indicate this.`;
      ...
      try {
        ...
        const openRouterResponse = await fetchWithTimeout(OPENROUTER_API_URL, {
          ...
          headers: {
            ...
            "HTTP-Referer": "https://glamstore.com",
            "X-Title": "GlamStore Beauty",
        ...
        const searchTerms = keywords.length > 2 ? keywords : [
          ...
          "serum", "cleanser", "mascara", "eyeshadow", "beauty", "cosmetic"
        ];
