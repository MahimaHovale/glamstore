      ...
      <header className="border-b sticky top-0 z-10 bg-background lg:hidden">
        <div className="flex h-16 items-center px-4">
          ...
          <div className="flex-1">
            <Link href="/admin" className="text-xl font-bold bg-gradient-to-r from-pink-600 to-purple-600 bg-clip-text text-transparent">
              GlamStore Admin
            </Link>
          </div>
              ...
              {sidebarItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={cn("h-4 w-4", isActive ? "text-pink-600 dark:text-pink-400" : "")} />
                    <span>{item.name}</span>
                    {isActive && <div className="ml-auto w-1 h-5 bg-pink-600 dark:bg-pink-400 rounded-full"></div>}
                  </Link>
                );
              })}
            </nav>
